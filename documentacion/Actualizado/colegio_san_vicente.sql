...
-- Tiempo de generación: 08-09-2020 a las 18:22:57
...
-- Procedimientos
--
CREATE DEFINER=`practica`@`%` PROCEDURE `agregarRolAdministrativo` (IN `NidUsuario` INT)  BEGIN
...
CREATE DEFINER=`practica`@`%` PROCEDURE `agregarRolAlumno` (IN `idUsuario` INT)  BEGIN
...
CREATE DEFINER=`practica`@`%` PROCEDURE `detallePersonas` (IN `Ndni` INT(9))  NO SQL
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `listarApoderados` ()  NO SQL
SELECT  idPersona, nombres, apellidoPaterno, apellidoMaterno
	FROM persona WHERE persona.idPersona 
    IN ( SELECT u.idPersona FROM usuario_rol_padre AS up
	-- INNER JOIN apoderado ON usuario_rol_padre.idPadre = apoderado.idPadre;
     INNER JOIN usuario AS u ON up.idUsuario = u.idUsuario)$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `listarAulas` ()  NO SQL
SELECT numeroAula,grado,nivel,seccion, vacantes,cantidad_matriculados FROM aula INNER JOIN grado on aula.idGrado=grado.idGrado INNER JOIN seccion on aula.idSeccion=seccion.idSeccion  
...
SELECT nombreAño,año,fechaInicio,fechaFin FROM anio_electivo p WHERE p.idPeriodo=idPeriodo$$

CREATE DEFINER=`practica`@`%` PROCEDURE `registrarAula` (IN `RnumeroAula` INT, IN `RidGrado` INT, IN `RidSeccion` INT, IN `Rvacantes` INT)  BEGIN
INSERT INTO aula(numeroAula, idGrado , idSeccion , vacantes) VALUES (RnumeroAula, RidGrado, RidSeccion, Rvacantes);

SELECT * FROM aula ORDER by idAula DESC LIMIT 1;
END$$

CREATE DEFINER=`practica`@`%` PROCEDURE `registrarPersona` (IN `Ndni` INT, IN `nombres` VARCHAR(30), IN `apellidoP` VARCHAR(20), IN `apellidoM` VARCHAR(30), IN `Nemail` VARCHAR(30), IN `Ntelefono` INT(9), IN `Ndireccion` VARCHAR(250), IN `NfechaNacimiento` DATE)  BEGIN
...
CREATE DEFINER=`practica`@`%` PROCEDURE `solicitaDatosPersona` (IN `idPersonaParam` INT(9))  NO SQL
SELECT * from persona where persona.idPersona=idPersonaParam$$

CREATE DEFINER=`practica`@`%` PROCEDURE `solicitarContraseniaMaster` (IN `correo` VARCHAR(50))  BEGIN
...
SELECT * FROM anio_electivo ORDER BY idPeriodo DESC LIMIT 1$$

CREATE DEFINER=`practica`@`%` PROCEDURE `solicitarRoles` (IN `idUsuarioParam` INT(20))  NO SQL
select idAdmin, idAlumno, idProfesor,idPadre from usuario WHERE idUsuario=idUsuarioParam$$

CREATE DEFINER=`practica`@`%` PROCEDURE `solicitarRolesT` (IN `idUsuarioParam` INT(20))  NO SQL
select idAdmin, idAlumno, idProfesor,idPadre from usuario WHERE usuario=idUsuarioParam$$
...
--
-- Estructura de tabla para la tabla `anio_electivo`
--
...
--
-- Volcado de datos para la tabla `anio_electivo`
--

INSERT INTO `anio_electivo` (`idPeriodo`, `nombreAño`, `año`, `fechaInicio`, `fechaFin`) VALUES
...
CREATE TABLE `aula` (
  ...
  `cantidad_matriculados` int(11) NOT NULL DEFAULT '0'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
INSERT INTO `aula` (`idAula`, `numeroAula`, `idGrado`, `idSeccion`, `vacantes`, `cantidad_matriculados`) VALUES
(1, 27, 1, 1, 40, 10),
(2, 4, 2, 1, 40, 20),
(3, 14, 3, 1, 40, 5),
(4, 14, 4, 2, 40, 1),
...
INSERT INTO `periodo` (`idBimestre`, `ordenBimestre`, `año`, `fechaInicio`, `fechaFin`) VALUES
...
(163, 73885757, 'ALVARO MARTÍN', 'BEGAZO', 'CARHUAYO', 'alvaro15@gmail.com', 987654321, '2020-09-17', 'Av canevaro'),
(164, 9128460, 'SEGUNDO', 'BEGAZO', 'MOLINA', 'segundo@gmail.com', 987654321, '2020-09-09', 'ad'),
(165, 10036495, 'YOLANDA ARCENIA', 'CARHUAYO', 'ESPINO DE BEGAZO', 'yola@gm', 94321, '2020-09-25', 'asd'),
(166, 73885752, 'CLAUDIA ROSA', 'BEGAZO', 'CARHUAYO', 'claudia@gmail.com', 987654321, '2020-09-19', 'asd');
...
(152, 73885757, '$2b$10$0Excfy7PeuTLHy1hcb6T3OZy23U8c7TOopqEO5dJuFJF0PVfKm7t6', 0, 0, 0, 69, 163),
(153, 9128460, '$2b$10$ALGW.Kf9lM8i4Zev06ZXEO5MDdwqz.qZ1vMKZK/pzGI.gPKbKO1Qq', 0, 0, 0, 70, 164),
(154, 10036495, '$2b$10$EQLfjYXo4OgvvFWVEJiTsehgQkyo8d6svOrTeJBSEENXgoPO92yIG', 0, 0, 0, 71, 165),
(155, 73885752, '$2b$10$bbWFF0rP8UiCHhFVhQFt3O49pIeHFy.LagS/i.8OaZz4KmlMibKxO', 0, 0, 0, 72, 166);
...
--
-- Indices de la tabla `anio_electivo`
--
ALTER TABLE `anio_electivo`
...
--
-- AUTO_INCREMENT de la tabla `anio_electivo`
--
ALTER TABLE `anio_electivo`
...
--
ALTER TABLE `aula`
  MODIFY `idAula` int(20) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=42;
...
--
ALTER TABLE `matricula`
  MODIFY `idMatricula` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=57;
...
--
ALTER TABLE `persona`
  MODIFY `idPersona` int(100) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=167;
...
--
ALTER TABLE `usuario`
  MODIFY `idUsuario` int(100) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=156;
...
--
ALTER TABLE `usuario_rol_padre`
  MODIFY `idPadre` int(20) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=73;
